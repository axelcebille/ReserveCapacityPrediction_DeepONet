...
class SimulationData:
    ...
    def generate_coordinate_features(self):
        ...
        for section_group in self.section_groups:
            for loading_protocol_number, loading_protocol in enumerate(self.cfg.loading_protocols):
                ...
                resisting_moment = self.data[f'{section_group}/{self.cfg.length_group}/{loading_protocol}/{self.cfg.ratio_limit_group}/Reaction/bot_RM1']
                horizontal_displacement = self.data[f'{section_group}/{self.cfg.length_group}/{loading_protocol}/{self.cfg.ratio_limit_group}/Displacement/top_U2']
                vertical_displacement = self.data[f'{section_group}/{self.cfg.length_group}/{loading_protocol}/{self.cfg.ratio_limit_group}/Displacement/top_U3']
                rotation_displacement = self.data[f'{section_group}/{self.cfg.length_group}/{loading_protocol}/{self.cfg.ratio_limit_group}/Displacement/top_R1']
                ...
                for i in range(1, len(indices)):
                    ...
                    self.features[section_group]['resisting_moments'].append(resisting_moment[indices[i]])
                    self.features[section_group]['displacements'].append([horizontal_displacement[indices[i]], vertical_displacement[indices[i]], rotation_displacement[indices[i]]])
    ...
    def features_to_numpy(self):
        for section_group in self.section_groups:
            ...
            self.features[section_group]['resisting_moments'] = np.array(self.features[section_group]['resisting_moments'])
            self.features[section_group]['displacements'] = np.array(self.features[section_group]['displacements'])
    ...
    def transform_disp_into_deformation(self):
        print('Transforming displacements into deformations.')
        for section_group in self.section_groups:
            self.features[section_group]['displacements'] = self.features[section_group]['displacements'] / int(self.cfg.length_group)
        print('Deformation features generated.')
        return None
    ...
    def filter_reserve_capacities(self, min_abs_rc):
        for section_group in self.section_groups:
            ...
            resisting_moments = self.features[section_group]['resisting_moments']
            resisting_moments = np.array(resisting_moments)
            displacements = self.features[section_group]['displacements']
            ...
            self.features[section_group]['resisting_moments'] = self.features[section_group]['resisting_moments'][combined_condition]
            self.features[section_group]['displacements'] = self.features[section_group]['displacements'][combined_condition]
