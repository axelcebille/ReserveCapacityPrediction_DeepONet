...
#file_path = "ReserveCapacityPrediction_DeepONet/figures/tsne_analysis/residual_capacity"
...
#file_path = "ReserveCapacityPrediction_DeepONet/figures/tsne_analysis/residual_capacity(no_transfo)"
...
file_path = "ReserveCapacityPrediction_DeepONet/figures/tsne_analysis/residual_capacityFNN"
...
#file_path = "ReserveCapacityPrediction_DeepONet/figures/tsne_analysis/residual_capacity(no_transfo)FNN"
...
def tsne_analysisRC(data, model, device, var, network_type, file_path):
    ...
    with torch.no_grad():
        for graph, trunk_input, target, in_flag, out_flag, col in loader:
            ...
            if var == "local_slenderness_ratio_web":
                lsr  = d / t_w # web lsr
                lsr_list.append(lsr)
            elif var == "local_slenderness_ratio_flange":
                lsr  = (b_f - t_w) / (2 * t_f) # flange lsr 
                lsr_list.append(lsr)
    ...
    if var == "residual_capacity":
        ...
        var_list = lsr_list
    elif var == "local_slenderness_ratio_web":
        var_list = lsr_list
    elif var == "local_slenderness_ratio_flange":
        var_list = lsr_list
    ...
    plt.savefig(f"{file_path}/tsne_latent_space_{network_type}_{var}.png", dpi=300, bbox_inches="tight")
    plt.close()

    print("✅ Figures saved in ./figures/tsne_analysis/ folder.")

# === RUN T-SNE ANALYSIS FOR RESIDUAL CAPACITY MODEL ===
variables = ["residual_capacity", "local_slenderness_ratio_web", "local_slenderness_ratio_flange", "local_slenderness_ratio"]
network_types = ["branch", "trunk"]

for var in variables:
    for network_type in network_types:
        tsne_analysisRC(data, model, device, var, network_type, file_path)
