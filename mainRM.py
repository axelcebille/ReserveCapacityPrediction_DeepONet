...
from src.utils import compute_mean_target, compute_std_target, compute_stats_by_column_moment,means_and_stds, compute_moment_by_column, elastic_moments_batch
...
model = DeepONet(branch_in=7, trunk_in=6+1, hidden_dim=264,hidden_dim_trunk=264, latent_dim=128, out_dim=1, dropout=0.2).to(device)
...
for epoch in range(n_epochs):
    ...
    for graphs, trunk_inputs, targets, _, _, col in train_loader:
        ...
        target_mean,target_std = means_and_stds(stats_target, col)
    ...
    if avg_val_loss < best_val_loss:
        ...
        patience_counter = 0
        torch.save(model.state_dict(), 'best_deeponet_model_RM.pth')
    else:
        patience_counter += 1
...
model.load_state_dict(torch.load('best_deeponet_model_RM.pth'))
model.eval()
...
with torch.no_grad():
    for graphs, trunk_inputs, targets, _, _, col in test_loader:
                ...
                graphs_trans = transform_batch_independent(graphs).to(device)
                targets = targets.reshape(-1,1)
                #batch_moments = elastic_moments_batch(elastic_moments, col)
                #batch_moments = batch_moments.to(device)
                #targets = targets*batch_moments
                
                target_mean,target_std = means_and_stds(stats_target, col)
                target_mean,target_std = target_mean.to(device),target_std.to(device)
                #targets = (targets - target_mean) / (target_std + 1e-12) 
                preds = model(graphs, trunk_inputs)
                #preds = preds*batch_moments
                preds = preds * (target_std + 1e-12) + target_mean
                preds_transfo = model(graphs_trans, trunk_inputs)
                #preds_transfo = preds_transfo*batch_moments
