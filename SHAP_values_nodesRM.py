...
class DeepONetSHAPWrapper(torch.nn.Module):
    """
    SHAP-compatible wrapper for graph-input DeepONet
    with an FNN branch operating on graph.x.
    """
    def __init__(
        self,
        model,
        ...
        self.node_feature_shape = node_feature_shape  # explicit, no inference
    ...
    def forward(self, x_flat):
        ...
        for i in range(B):
            ...
            y = y.view(-1)
            outputs.append(y[self.output_component])

        return torch.stack(outputs, dim=0).unsqueeze(-1)  # (B, 1)
...
background = torch.cat(background_x, dim=0)  # (B, N*F)
...
for col_type in data.features.keys():
    ...
    for graph, trunk_input, target, in_flag, out_flag, col in loader:
        ...
        # Update trunk input for current sample
        shap_model.trunk_input = trunk_input

        # SHAP perturbs ONLY node features
        x_flat = graph.x.view(1, -1)

        shap_vals = explainer.shap_values(x_flat)[0]  # numpy (1, N*F)
        shap_vals = torch.tensor(shap_vals, device=device)

        # Restore node-feature structure
        shap_vals = shap_vals.view(graph.x.shape)  # (N, F)

        # Node-level importance (still meaningful!)
    ...
    make_importance_gif_dynamic_graph(data, importance_list, col_type,
                                                gif_path=f"ReserveCapacityPrediction_DeepONet/figures/gifs_SHAP_importanceRM/RM_SHAP_importance_col{col_type}.gif",
                                                elev=30, azim=60)
