...
def test_rc(model,test_loader,criterion,target_mean,target_std,device,target_normalize=True):
    # --- Test ---
    model.eval()
    val_loss = 0.0
    test_loss = 0.0
...
def test_model_rc(data, model, device, target_normalize=False, features_normalize=True, weights_name="best_deeponet_model_RC"):
    ...
    avg_test_loss, mape_loss, mape_transfo_loss = test_rc(model,test_loader,criterion,target_mean,target_std,device,target_normalize=target_normalize)
...
for i, name in enumerate(weight_names):
    ...
    mape_loss, mape_transfo_loss = test_model_rc(data, model_list[i], device, target_normalize, 
